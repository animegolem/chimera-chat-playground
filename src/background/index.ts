...
import { TabGroupInfo, TabInfo, ModelInfo } from '@/shared/types';

console.log(`${EXTENSION_INFO.NAME} ${EXTENSION_INFO.VERSION} background script loaded`);
...
if (FEATURE_FLAGS.ENABLE_TAB_NAMING && typeof browser.tabs.group !== 'undefined') {
  ...
  console.log('Tab groups API not available - extension will work without auto-naming');
}
...
async function handleMessage(message: SidebarMessage, sendResponse: (response: ResponseMessage) => void) {
  try {
    switch (message.type) {
      case 'GET_TAB_GROUPS':
      ...
      case 'RENAME_TAB_GROUP':
        await handleRenameTabGroup(message.groupId!, message.name!, sendResponse);
        break;
        
      case 'UPDATE_MODEL_SETTINGS':
        await handleUpdateModelSettings(message.modelId!, message.modelSettings!, sendResponse);
        break;
...
// Message handlers
async function handleGetTabGroups(sendResponse: (response: ResponseMessage) => void) {
  try {
...
async function handleRenameTabGroup(groupId: number, name: string, sendResponse: (response: ResponseMessage) => void) {
  try {
    if (typeof browser.tabs.group === 'undefined') {
      sendResponse({ success: false, error: 'Tab groups not available in this Firefox version' });
      return;
    }
    
    // TODO: Implement when tab groups API is stable in Firefox
    sendResponse({ success: false, error: 'Tab group renaming not yet implemented' });
  } catch (error) {
    sendResponse({ 
      success: false, 
...
async function handleUpdateModelSettings(modelId: string, updates: Partial<ModelInfo>, sendResponse: (response: ResponseMessage) => void) {
  try {
...
async function handleToggleModel(modelId: string, sendResponse: (response: ResponseMessage) => void) {
  try {
...
// Helper functions
async function generateTabGroupName(tabData: Array<{title: string, url: string}>): Promise<string> {
  ...
  const titles = tabData.map(t => t.title).join(' ').toLowerCase();
  
  // Development patterns
  if (titles.includes('github') || titles.includes('gitlab') || titles.includes('bitbucket')) {
    return 'Development';
  }
  
  // Documentation patterns
  if (titles.includes('docs') || titles.includes('documentation') || titles.includes('api')) {
    return 'Documentation';
  }
  
  // Learning patterns
  if (titles.includes('tutorial') || titles.includes('learn') || titles.includes('course')) {
    return 'Learning';
  }
  
  // Shopping patterns
  if (titles.includes('shop') || titles.includes('buy') || titles.includes('cart')) {
    return 'Shopping';
  }
  
  // Social media patterns
  if (titles.includes('twitter') || titles.includes('linkedin') || titles.includes('facebook')) {
    return 'Social';
  }
  
  // News patterns
  if (titles.includes('news') || titles.includes('article') || titles.includes('blog')) {
    return 'News';
  }
