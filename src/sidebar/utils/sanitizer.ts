...
export class ContentSanitizer {
  ...
  private createSecureRenderer(): Renderer {
    ...
    renderer.code = (code: string, language?: string) => {
      ...
      const safeLanguage = language ? this.validateLanguage(language) : '';
  ...
  private validateImageUrl(url: string): string {
    try {
      ...
      if (parsed.protocol === 'data:' && url.startsWith('data:image/')) {
        const matches = url.match(/^data:image\/(png|jpg|jpeg|gif|svg\+xml|webp);base64,/);
        if (matches) {
          return url;
        }
      }
  ...
  private validateLanguage(language: string): string {
    const allowedLanguages = [
      ...
      'bash', 'shell', 'powershell', 'sql', 'graphql', 'dockerfile'
    ];
